'use client'
/**
 * ProfileClient — the ONLY client component for the profile page.
 *
 * It receives the already-fetched `profile` from the server component
 * (page.tsx) as a prop. This means:
 *   - No useEffect / fetch on the client
 *   - No loading spinner / skeleton
 *   - The full page HTML is sent by the server instantly
 *   - Only tab-switching, copy-link, share, and lightbox need JS
 *
 * Every style, colour and layout is identical to the original.
 */
...
export default function ProfileClient({ profile }: { profile: any }) {
        ...
        <tbody>
          {rates.map((r, i) => (
            <tr
              ...
              style={{
                borderBottom:
                  i < rates.length - 1 ? '1px solid rgba(255,255,255,0.05)' : 'none',
              }}
            >
  ...
  return (
    ...
    >
      ...
      {/* BANNER */}
      <div style={{ position: 'relative', height: 120, background: '#0e0e1c', zIndex: 10 }}>
        <div
          style={{
            position: 'absolute',
            inset: 0,
            backgroundImage:
              'repeating-linear-gradient(0deg,transparent,transparent 30px,rgba(255,107,43,0.06) 30px,rgba(255,107,43,0.06) 31px),repeating-linear-gradient(90deg,transparent,transparent 30px,rgba(255,107,43,0.06) 30px,rgba(255,107,43,0.06) 31px)',
          }}
        ></div>
        <div
          style={{
      ...
      <div style={{ maxWidth: 680, margin: '0 auto', padding: '0 16px 48px' }}>
        ...
        >
          <div style={{ position: 'relative', display: 'inline-block' }}>
            {/*
              FIX 3 — Next/Image with priority for the avatar.
              This tells the browser to preload this image (it's the LCP element on mobile)
              and serves WebP/AVIF automatically. Design is identical.
            */}
            <div
              style={{
                width: 88,
            ...
            >
              {profile.photo_url ? (
                // fetchPriority="high" tells the browser to preload this as LCP image
                // Same as original <img> — no visual change, just loads earlier
                // eslint-disable-next-line @next/next/no-img-element
                <img src={profile.photo_url} alt={profile.full_name} fetchPriority="high" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
              ) : (
                initials
              )}
            </div>
          ...
          <div style={{ display: 'flex', gap: 6, paddingBottom: 4 }}>
            <button
              ...
              style={{
                ...
                gap: 5,
                padding: '7px 12px',
                background: copied ? 'rgba(34,197,94,0.12)' : '#111120',
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 12, marginBottom: 12 }}>
          {[
            profile.city && { icon: 'ti-map-pin', text: profile.city },
            profile.languages && { icon: 'ti-language', text: profile.languages },
            profile.niche && { icon: 'ti-category', text: profile.niche },
          ]
            .filter(Boolean)
            .map((m: any) => (
              <span
                key={m.text}
                style={{ display: 'flex', alignItems: 'center', gap: 4, fontSize: 11, color: 'rgba(255,255,255,0.3)' }}
              >
                <i className={`ti ${m.icon}`} style={{ fontSize: 12 }}></i>
                {m.text}
              </span>
            ))}
        </div>
        ...
        >
          {[
            ...
            <div
              key={s.label}
              className="ik-bento"
              style={{ ...S.card, padding: '14px 12px', textAlign: 'center', transition: 'all 0.2s' }}
            >
              <i
                className={`ti ${s.icon}`}
                style={{ fontSize: 14, color: 'rgba(255,107,43,0.5)', display: 'block', marginBottom: 6 }}
              ></i>
              <div
                style={{
                  fontFamily: "'Syne',sans-serif",
        ...
        >
          {TABS.map((t) => (
            <button
              key={t.id}
              onClick={() => setTab(t.id)}
              className="ik-tab ik-tab-btn"
              style={{
                padding: '11px 16px',
                background: 'none',
                border: 'none',
                borderBottom: tab === t.id ? '2px solid #FF6B2B' : '2px solid transparent',
                color: tab === t.id ? '#FF6B2B' : 'rgba(255,255,255,0.3)',
                fontSize: 12,
            ...
            {(platformList.length > 0 || profile.audience_age) && (
              <div
                style={{
                  display: 'grid',
                  gridTemplateColumns:
                    platformList.length > 0 && profile.audience_age ? '1fr 1fr' : '1fr',
                  gap: 8,
                  marginBottom: 8,
                }}
              >
                  ...
                  <div className="ik-bento" style={{ ...S.card, padding: '18px 16px', transition: 'all 0.2s' }}>
                    ...
                    <div style={{ marginBottom: 12 }}>
                      ...
                      {(
                        [
                          ['13–17', 6],
                          ['18–24', profile.audience_age === '18-24' ? 66 : 18],
                          ['25–34', profile.audience_age === '25-34' ? 66 : 20],
                          ['35+', profile.audience_age === '35+' ? 66 : 8],
                        ] as [string, number][]
                      ).map(([l, p]) => (
                        ...
                        {cities.slice(0, 3).map((city: string, i: number) => {
                          ...
                          return (
                            <div key={city} style={{ display: 'flex', alignItems: 'center', gap: 7, marginBottom: 5 }}>
                              ...
                              <span style={{ fontSize: 10, color: '#FF6B2B', minWidth: 24, textAlign: 'right' }}>
                                {pct}%
                              </span>
                            </div>
                          )
                    ...
                    {profile.follower_growth && (
                      ...
                      >
                        <i className="ti ti-trending-up" style={{ fontSize: 12 }}></i>
                        {profile.follower_growth}
                      </div>
                    )}
                  </div>
              ...
              <div className="ik-bento" style={{ ...S.card, padding: '18px 16px', marginBottom: 8, transition: 'all 0.2s' }}>
                ...
                <div
                  style={{
                    ...
                    flexWrap: 'wrap',
                    gap: 7,
                    marginBottom: profile.best_campaign ? 14 : 0,
                  }}
                >
                  {brands.map((b: string) => (
                    <span key={b} style={S.brandPill}>
                      {b}
                    </span>
                  ))}
                </div>
            ...
            {portfolioItems.length > 0 && (
              <div className="ik-bento" style={{ ...S.card, padding: '18px 16px', marginBottom: 8, transition: 'all 0.2s' }}>
                <div style={S.sectionTitle}>
                  My work{' '}
                  <span style={{ color: 'rgba(255,255,255,0.2)', fontWeight: 400, fontSize: 9 }}>
                    — tap to expand
                  </span>
                </div>
                <div
                  style={{
                    display: 'grid',
                    gridTemplateColumns: portfolioItems.length === 1 ? '1fr' : '1fr 1fr',
                    gap: 8,
                  }}
                >
                ...
                {profile.vibe && (
                  <div style={{ marginBottom: skillsList.length > 0 ? 14 : 0 }}>
                    ...
                    <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
                      {[profile.niche, profile.vibe].filter(Boolean).map((v: string) => (
            ...
            {(profile.collab_type || profile.response_time || profile.turnaround) && (
              <div className="ik-bento" style={{ ...S.card, padding: '18px 16px', marginBottom: 8, transition: 'all 0.2s' }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 8 }}>
                  {[
                    ...
                    .map((item: any) => (
                      <div
                        key={item.label}
                        style={{ background: '#0e0e1e', borderRadius: 10, padding: '10px 12px', textAlign: 'center' }}
                      >
                        <div
                          style={{
                            fontSize: 9,
                ...
                {profile.categories_avoid && (
                  ...
                  >
                    <span style={{ fontSize: 11, color: 'rgba(255,120,120,0.7)' }}>
                      <i className="ti ti-ban" style={{ marginRight: 5 }}></i>Won&apos;t work with:{' '}
                      {profile.categories_avoid}
                    </span>
                  </div>
                )}
                  ...
                  .map((item: any, i: number, arr: any[]) => (
                    <div key={item.label}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '10px 0' }}>
                        ...
                        <div>
                          <div style={{ fontSize: 10, color: 'rgba(255,255,255,0.25)', marginBottom: 2 }}>
                            {item.label}
                          </div>
                          {item.href ? (
                            <a
                      ...
                      {i < arr.length - 1 && (
                        <div style={{ height: 1, background: 'rgba(255,255,255,0.04)' }}></div>
                      )}
                    </div>
                  ))}
              </div>
        ...
        {tab === 'mediakit' && (
          <div style={{ animation: 'fadeUp 0.2s ease', display: 'flex', flexDirection: 'column', gap: 8 }}>
            <div
              style={{
                background: 'linear-gradient(135deg,#1a0800,#0e0e1c)',
                border: '1px solid rgba(255,107,43,0.15)',
                borderRadius: 16,
                padding: '20px',
                position: 'relative',
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: 18 }}>
                ...
                <div>
                  ...
                  <div style={{ fontSize: 11, color: 'rgba(255,255,255,0.35)' }}>
                    {profile.niche} · {profile.city} · {profile.languages}
                  </div>
                </div>
              </div>
              <div
              ...
              >
                {[
                  ...
                  <div key={l} style={{ textAlign: 'center' }}>
                    <div style={{ fontFamily: "'Syne',sans-serif", fontSize: 18, fontWeight: 800, color: '#FF6B2B' }}>
                      {n}
                    </div>
                    <div
                      style={{
                ...
                {platformList.map((p: string) => {
                  ...
                  const followers =
                    p === 'Instagram'
                      ...
                      : null
                  const stat2 =
                    p === 'Instagram' ? profile.engagement_rate : p === 'YouTube' ? profile.avg_views : null
                      ...
                      <div style={{ display: 'flex', gap: 10 }}>
                        {followers && (
                          <div style={{ textAlign: 'right' }}>
                            ...
                            <div style={{ fontSize: 9, color: 'rgba(255,255,255,0.2)' }}>followers</div>
                          </div>
                        )}
                        {stat2 && (
                          <div style={{ textAlign: 'right' }}>
                            ...
                            <div style={{ fontSize: 9, color: 'rgba(255,255,255,0.2)' }}>{stat2label}</div>
                          </div>
                        )}
                      </div>
                    </div>
            ...
            <div style={{ ...S.card, padding: '16px' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 8 }}>
                {[
                  ...
                  <div
                    key={c.label}
                    style={{ background: '#0e0e1e', borderRadius: 10, padding: '12px', textAlign: 'center' }}
                  >
                    <i
                      className={`ti ${c.icon}`}
                      style={{ fontSize: 22, color: c.color, display: 'block', marginBottom: 6 }}
                    ></i>
              ...
              <div style={{ ...S.card, padding: '16px' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                  ...
                  {profile.custom_package && (
                    ...
                    >
                      <div>
                        <div style={{ fontSize: 13, fontWeight: 600, color: '#fff' }}>Full Bundle</div>
            ...
            >
              ...
              >
                Identity Kit · identitykit.in/{profile.username}
              </span>
              ...
              >
                <div>
                  ...
                  <div style={{ fontSize: 12, color: 'rgba(255,255,255,0.3)', marginTop: 3 }}>
                    {profile.niche} · identitykit.in/{profile.username}
                  </div>
                </div>
                {profile.rate_valid_till && (
                  ...
                  >
                    Valid till{' '}
                    {new Date(profile.rate_valid_till + '-01').toLocaleString('default', {
                      month: 'long',
                      year: 'numeric',
                    })}
                  </span>
            ...
            {profile.custom_package && (
              ...
              >
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  ...
                  <div style={{ fontFamily: "'Syne',sans-serif", fontSize: 22, fontWeight: 800, color: '#FF6B2B' }}>
                    Best value
                  </div>
                </div>
              </div>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 14 }}>
                  ...
                  {igHandle && (
                    <span style={{ fontSize: 11, color: 'rgba(255,255,255,0.2)' }}>
                      @{igHandle} · {profile.instagram_followers}
                    </span>
                  )}
                </div>
            ...
            {ytRates.length > 0 && (
              <div style={{ ...S.card, padding: '18px 16px' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 14 }}>
                  ...
                  {ytHandle && (
                    <span style={{ fontSize: 11, color: 'rgba(255,255,255,0.2)' }}>
                      {ytHandle} · {profile.youtube_subscribers}
                    </span>
                  )}
                </div>
                  ...
                  {selectedTerms.map((t) => (
                    <div key={t.key} style={{ display: 'flex', gap: 10, marginBottom: 8, alignItems: 'flex-start' }}>
                      <i
                        ...
                        style={{ fontSize: 14, color: '#FF6B2B', flexShrink: 0, marginTop: 1 }}
                      ></i>
        ...
        {tab === 'cv' && (
          ...
          >
            ...
            >
              ...
              <div style={{ flex: 1 }}>
                ...
                <div style={{ fontSize: 12, color: 'rgba(255,255,255,0.35)', marginBottom: 8 }}>
                  {profile.niche} Creator · {profile.platforms?.split(',').slice(0, 2).join(' & ')}
                </div>
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 10 }}>
                  {[
                    profile.city && { icon: 'ti-map-pin', val: profile.city },
                    profile.email && { icon: 'ti-mail', val: profile.email },
                    igHandle && { icon: 'ti-brand-instagram', val: `@${igHandle}` },
                  ]
                    .filter(Boolean)
                    .map((m: any) => (
                      <span
                      ...
                      >
                        <i className={`ti ${m.icon}`} style={{ fontSize: 11, color: '#FF6B2B' }}></i>
                        {m.val}
                      </span>
                    ))}
                </div>
                ...
                <div style={{ marginBottom: 18 }}>
                  ...
                  {[
                    ...
                    .map(([val, key]) => (
                      <div
                        key={key as string}
                        style={{ background: '#08080E', borderRadius: 8, padding: '7px 10px', marginBottom: 6 }}
                      >
                        <div style={{ fontFamily: "'Syne',sans-serif", fontSize: 14, fontWeight: 800, color: '#FF6B2B' }}>
                          {val}
                        </div>
                    ...
                    {platformList.map((p: string) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          >
                            <i className={`ti ${cfg.icon}`} style={{ color: cfg.color, fontSize: 12 }}></i>
                            {p}
                          </span>
                          {val && (
                            <span
                  ...
                  <div style={{ marginBottom: 18 }}>
                    ...
                    {brands.map((b: string, i: number) => (
                      <div
                        ...
                        style={{
                          paddingBottom: 10,
                          marginBottom: i < brands.length - 1 ? 10 : 0,
                          borderBottom: i < brands.length - 1 ? '1px solid rgba(255,255,255,0.04)' : 'none',
                        }}
                      >
                        ...
                        <div style={{ fontSize: 11, color: 'rgba(255,255,255,0.3)' }}>
                          Content creation · Brand collaboration
                        </div>
                      </div>
                    ))}
                  </div>
                  ...
                  <div style={{ marginBottom: 18 }}>
                    ...
                    <div style={{ display: 'flex', gap: 8 }}>
                      <i
                        ...
                        style={{ fontSize: 16, color: '#FCD34D', flexShrink: 0, marginTop: 1 }}
                      ></i>
                  ...
                  <div>
                    ...
                    {profile.turnaround && (
                      <div style={{ marginTop: 8, fontSize: 11, color: 'rgba(255,255,255,0.3)' }}>
                        ⏱ Turnaround: {profile.turnaround} · 1 free revision included
                      </div>
                    )}
                  </div>
                )}
            ...
            >
              ...
              {allRates.length > 0 && (
                <span style={{ fontSize: 11, color: '#FF6B2B' }}>
                  Starting ₹{Number(allRates[0].price).toLocaleString('en-IN')} per collab
                </span>
              )}
            </div>
          </div>
      ...
      >
        ...
        <a href="https://identitykit.in" style={{ fontSize: 11, color: '#FF6B2B', textDecoration: 'none' }}>
          identitykit.in
        </a>
      </div>
    </div>
